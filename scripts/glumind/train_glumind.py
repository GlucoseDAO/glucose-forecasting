...
def normalize_study_groups_column(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize study_group labels in-place-safe form."""
    if df.empty:
        return df
    out = df.copy()
    out["study_group"] = out["study_group"].astype(str).map(normalize_study_group_label)
    return out
...
def load_splits_streaming(
    ...
    """Streaming CSV loader — returns (train, val, test) DataFrames."""
    uid_col = COL_SEQ if unique_id_choice == "sequence_id" else COL_USER
    usecols = [uid_col, COL_TS, COL_SPLIT, COL_GROUP, COL_EVENT,
    ...
    for chunk in pd.read_csv(csv_path, usecols=usecols,
        ...
        chunk[COL_TS] = pd.to_datetime(chunk[COL_TS], format=TS_FORMAT,
                                       errors="coerce")
...
def apply_split_scheme(
    ...
    if split_scheme == "trainval_test_as_val":
        ...
        # In this mode there is no held-out test set.
...
def impute_and_sort(df: pd.DataFrame) -> pd.DataFrame:
    """Sort by (unique_id, ds), forward-fill then back-fill numeric columns."""
    if df.empty:
        return df
    ...
    df[["glucose", "hr", "steps"]] = df[["glucose", "hr", "steps"]].fillna(0.0).astype(np.float32)
    return df


def limit_series(df: pd.DataFrame, max_series: int) -> pd.DataFrame:
    if df.empty or max_series <= 0:
        return df
...
class GlucoseWindowDataset(Dataset):
    """Sliding-window dataset for multimodal glucose forecasting."""
    ...
    def __init__(
        ...
        if fit_scalers or scaler_glucose is None:
            all_g = np.concatenate(all_glucose).reshape(-1, 1)
            all_h = np.concatenate(all_hr).reshape(-1, 1)
            all_s = np.concatenate(all_steps).reshape(-1, 1)
        ...
        n_skipped = 0
        for uid, grp in df.groupby("unique_id", sort=False):
            g = self.scaler_glucose.transform(all_glucose[idx].reshape(-1, 1)).ravel()
            h = self.scaler_hr.transform(all_hr[idx].reshape(-1, 1)).ravel()
            s = self.scaler_steps.transform(all_steps[idx].reshape(-1, 1)).ravel()
            sg = grp["study_group"].iloc[0]

            total = len(g)
            n_windows = total - input_steps - horizon + 1
            if n_windows <= 0:
                n_skipped += 1
            ...
            for i in range(max(0, n_windows)):
                ...
                y = g[i + input_steps : i + input_steps + horizon]  # (horizon,)
                self.windows_x.append(x.astype(np.float32))
                self.windows_y.append(y.astype(np.float32))
    ...
    def __getitem__(self, idx):
        return (
            torch.from_numpy(self.windows_x[idx]),
            torch.from_numpy(self.windows_y[idx]),
        )
...
def compute_and_print_metrics(
    ...
    pd.DataFrame([{"mae": mae, "rmse": rmse, "mard": mard}]).to_csv(
    ...
    if dataset is not None and len(dataset.study_groups) == len(true_arr):
        ...
        for g in sorted(set(groups)):
            ...
            if not mask.any():
                continue
            tg = true_arr[mask].ravel().reshape(-1, 1)
            pg = pred_arr[mask].ravel().reshape(-1, 1)
            tg_inv = scaler_glucose.inverse_transform(tg).ravel()
            pg_inv = scaler_glucose.inverse_transform(pg).ravel()
            m, r, md = mae_rmse_mard(tg_inv, pg_inv)
            rows.append({"study_group": g, "n_windows": int(mask.sum()),
                          "mae": m, "rmse": r, "mard": md})

        by_group = pd.DataFrame(rows).sort_values("mae").reset_index(drop=True)
        print(f"\n=== {split_name.upper()} METRICS (by Study Group) ===")
        print(by_group.to_string(index=False))
        by_group.to_csv(run_dir / f"{split_name}_metrics_by_study_group.csv",
                        index=False)
...
def update_latest_symlink(run_dir: Path, out_dir: Path):
    """Updates 'latest' symlink to point to the new run_dir."""
    try:
        out_dir.mkdir(parents=True, exist_ok=True)
        symlink_path = out_dir / "latest"
        if symlink_path.exists() or symlink_path.is_symlink():
            symlink_path.unlink()
        # Use a relative link so nested run directories resolve correctly.
        rel_target = run_dir.relative_to(out_dir)
        symlink_path.symlink_to(rel_target)
        print(f"Latest run symlink: {symlink_path} -> {rel_target}")
    except Exception as e:
...
def mode_per_group(train_df, val_df, test_df, args, device):
    ...
    print("\n=== MODE: PER_GROUP ===")
    groups = [g for g in STUDY_GROUP_ORDER
              if g in train_df["study_group"].unique()]

    for group in groups:
        print(f"\n--- Group: {group} ---")
        tr = train_df[train_df["study_group"] == group]
        va = val_df[val_df["study_group"] == group] if not val_df.empty else val_df
        te = test_df[test_df["study_group"] == group] if not test_df.empty else test_df
...
def mode_cohort_wise(train_df, val_df, test_df, args, device):
    ...
    for group in groups:
        ...
        tr = train_df[train_df["study_group"] == group]
        va = val_df[val_df["study_group"] == group] if not val_df.empty else val_df
        # Evaluate on ALL test groups to measure cross-cohort performance
        te = test_df if not test_df.empty else test_df

        if tr.empty:
            print(f"  No training data for {group}, skipping.")
            continue

        # Build datasets: fit scalers on THIS cohort's train data
        train_ds, val_ds, test_ds = build_datasets(tr, va, te, args)
...
def mode_continual(train_df, val_df, test_df, args, device):
    ...
    # We need a globally-fitted scaler for consistent evaluation
    ...
    for i, group in enumerate(groups):
        ...
        if not val_df.empty:
            if args.continual_val_scope == "all_groups":
                ...
                va = val_df[val_df["study_group"] == group]
        else:
            va = val_df
        ...
        # Use global scalers for consistent evaluation
        train_ds = GlucoseWindowDataset(
        ...
        # Snapshot current model as teacher for next cohort
...
def main():
    ...
    if args.study_groups:
        ...
        train_df = train_df[train_df["study_group"].isin(groups)]
        val_df = val_df[val_df["study_group"].isin(groups)]
        test_df = test_df[test_df["study_group"].isin(groups)]
    ...
    print(f"Study groups in train: "
          f"{sorted(train_df['study_group'].unique().tolist())}")
