...
This is a separate, self-contained experiment folder — nothing in `scripts/sugar_one/`,
`scripts/glumind*/`, or `scripts/common/` is modified, only imported.
...
`scripts/sugar_jepa/pretrained/cgm_jepa/` (no network needed), stays **frozen** by default. The
...
[`scripts/sugar_one/tune_sugar_one_full.toml`](../sugar_one/tune_sugar_one_full.toml) — the best-tuned
...
- Everything else (multi-scale self-attention, flatten + output head, MSE loss, MinMax scalers for
  glucose/basal/bolus/carbs, imputation policy) is identical to `scripts/sugar_one/sugar_one_model.py` /
  `train_sugar_one.py`.
...
root-caused and fixed (see `scripts/common/network.py`), so Hub-based loading works fine there too. The
vendored copy under `scripts/sugar_jepa/pretrained/cgm_jepa/` (`config.json` + `model.safetensors`, ~4MB,
