...
`src/sugar_jepa/pretrained/cgm_jepa/` rather than fetched at runtime. This was originally a
workaround for a Windows TLS problem on this dev machine — since root-caused and fixed in
`src/common/network.py` (`apply_windows_tls_workarounds()`, called from `JepaEncoderWrapper` before
...
all, so there's no reason to prefer a Hub fetch for this proof of concept. See `src/common/network.py`
