#!/usr/bin/env python3
"""
SugarJepa-2 — SugarOne + **our own** JEPA glucose encoder as a 4th
cross-attention auxiliary stream. See src/sugar_jepa/sugar_jepa_model.py
(``SugarJepaModel2``) and src/sugar_jepa/README.md.

Sibling of ``train_sugar_jepa.py``, which trains the variant built on the
*vendored, pretrained* CGM-JEPA encoder. Here the encoder is ours: a plain patch
transformer trained end-to-end, or warm-started from a self-supervised
checkpoint produced by ``jepa_pretrain.py`` (``--jepa-init``).

Dataset: data/input/loop_and_ai_ready/loop_ai_ready_joined2_dev.csv (or the full
...
from sugar_jepa.sugar_jepa2_spec import SUGAR_JEPA2_SPEC
...
from common.scalers import SCALERS_FILENAME, save_scalers_for_run
...
app = typer.Typer(
    name="train_sugar_jepa2",
    add_completion=False,
    help="SugarJepa-2: SugarOne + our own JEPA glucose-embedding stream.",
)
...
def make_model(cfg: dict, device: torch.device) -> SugarJepaModel2:
    ...
    if cfg["freeze_jepa"]:
        # An exact freeze, not an approximation: requires_grad=False keeps the
        # tensors out of the optimizer entirely, so AdamW's decoupled weight
        # decay cannot touch them either. eval() is belt-and-braces — the encoder
        # is built with jepa_dropout=0.0 and normalises with LayerNorm, so it has
        # no train/eval-dependent behaviour to begin with.
        enc = _jepa_encoder(model)
        for param in enc.parameters():
            param.requires_grad = False
        enc.eval()
        typer.echo("JEPA encoder FROZEN — its weights will not change (--jepa-lr ignored).")
...
def make_optimizer_and_scheduler(
    ...
    """Param groups: by default the pretrained-or-random JEPA encoder gets a
    smaller LR (`--jepa-lr`) than the freshly-initialised forecasting model around
    it. Under `--freeze-jepa` the encoder is left out of the optimizer entirely
    and there is only one group.

    Note that `--jepa-lr 0` is NOT the same thing. The scheduler's `eta_min` is a
    single value shared by every group, so a zero-base group anneals *upward* to
    `lr * 0.01` — by the last epoch the "frozen" encoder would be training at the
    backbone's final LR. Dropping the group is the only exact freeze.
    """
    ...
    frozen = cfg["freeze_jepa"]
    # A silent empty group means every param trains at the wrong LR — except when
    # the encoder is deliberately frozen, which is exactly an empty JEPA group.
    if not other_params or (jepa_params and frozen) or (not jepa_params and not frozen):
        raise RuntimeError(
            f"Optimizer param split is degenerate: {len(jepa_params)} trainable JEPA "
            f"tensors, {len(other_params)} others, freeze_jepa={frozen} — "
            "check _jepa_encoder()."
        )

    groups = [{"params": other_params, "lr": cfg["lr"]}]
    if not frozen:
        groups.append({"params": jepa_params, "lr": cfg["jepa_lr"]})
    ...
    if frozen:
        echo_plain(f"  Training 0 out of {_n(jepa_all):,} JEPA params (frozen)")
    else:
        echo_plain(
...
def run_train_and_eval(
    ...
    save_scalers_for_run(
        run_dir,
        kind=SUGAR_JEPA2_SPEC.kind,
        scalers=SUGAR_JEPA2_SPEC.extract_scalers(train_ds),
        provenance={
            "csv": str(cfg.get("csv", "")),
    ...
    meta = dict(cfg)
    meta.update({
        "model_type": SUGAR_JEPA2_SPEC.kind,
...
def _mode_global(
    ...
    run_name = f"sugar_jepa2_global_h{cfg['horizon']}_{ts}"
...
def main(
    ...
    freeze_jepa: bool = typer.Option(
        False,
        help="Freeze the JEPA encoder: it is dropped from the optimizer and its weights "
             "never change. --jepa-lr is then ignored. Only meaningful with --jepa-init.",
    ),
    device_name: str = typer.Option("cuda", "--device", help="cpu | mps | cuda."),
    seed: int = typer.Option(42, help="Random seed."),
    out_dir: Path = typer.Option(DEFAULT_RUNS_ROOT / "sugar_jepa2", help="Output directory."),
) -> None:
    """Train SugarJepa-2 (global mode only) — SugarOne + our own JEPA glucose encoder."""
    ...
    # Freezing a randomly initialised encoder pins the JEPA branch to noise for
    # the whole run. It is never what anyone means by "frozen encoder", and the
    # run would look like a quiet architecture failure rather than a flag mistake.
    if freeze_jepa and not jepa_init:
        raise typer.BadParameter(
            "--freeze-jepa without --jepa-init would freeze a randomly initialised "
            "encoder. Pass a pretrained encoder.pt, or drop --freeze-jepa."
        )
