...
Importing this module only exposes static metadata. NeuralForecast is imported
when a selected model is constructed.
"""
    ...
    *,
    ...
    if selection in suite_models:
        ...
        if not names or any(name is None for name in names):
            known = ", ".join(
                sorted((*suite_models, *(model.value for model in NeuralForecastModel)))
            )
            raise ValueError(
                f"unknown NeuralForecast model or suite {selection!r}; choose from {known}"
            )
...
def create_model(
    ...
    *,
    ...
    if definition.optional_dependency and find_spec(definition.optional_dependency) is None:
        raise RuntimeError(
            f"{definition.name.value} requires optional dependency "
            f"{definition.optional_dependency!r}; install it with "
            f"`uv add {definition.optional_dependency}`."
        )
