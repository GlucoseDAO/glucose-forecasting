...
MARD where we have them. The horizon is the same for every model.
...
chronological test. SugarOne and SugarJEPA-288 are scored on the dataset
\texttt{test} split. NBEATSx and TFT numbers are the holdout MAE of the
global NeuralForecast bundles used for continue-fit. These models are not
weak toys. Personal zero-shot MAE on the seven T1DM users is higher for
every model (Section~\ref{sec:paths}); that is a different test.
...
\bottomrule
\end{tabular}
\caption{Joined-corpus holdout (global test). SugarOne and SugarJEPA-288:
dataset \texttt{test} split. NBEATSx and TFT: global-bundle holdout MAE.
...
\citep{challu2023nhits}. LSTM is worse at 30 days (\(\Delta\) \(+6.00\)).
TiDE's 30-day mean \(\Delta\) is \(-7.01\) (helpful) but its global holdout
MAE is 15.90, well above the models in Table~\ref{tab:global}. We left them
off Figure~\ref{fig:curves} so the harmful-path contrast is one curve
(NBEATSx), not five.
